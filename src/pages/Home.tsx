...
const Home: React.FC = () => {
  ...
  const getData = async (sort: string = "", search: string = "") => {
    if (search === "") {
      if (sort === "asc") {
        await axios
          .get(`http://103.179.188.25:8000/students/score_asc`)
          .then((response) => {
            setData(response.data);
          });
      } else if (sort === "desc") {
        await axios
          .get(`http://103.179.188.25:8000/students/score_desc`)
          .then((response) => {
            setData(response.data);
          });
      } else {
        await axios
          .get(`http://103.179.188.25:8000/students/`)
          .then((response) => {
            setData(response.data);
          });
      }
      ...
      await axios
        .get(
          `http://103.179.188.25:8000/students/search?search=${search}&sort=${sort}`
        )
        .then((response) => {
          setData(response.data);
        });
  ...
  const postData = async (data: any) => {
    await axios
      .post("http://103.179.188.25:8000/students/create", data)
      .then((response) => {
  ...
  const putData = async (id: string, data: any) => {
    await axios
      .put("http://103.179.188.25:8000/students/" + id, data)
      .then((response) => {
        console.log(response.data);
        // setData(response.data);
      })
  ...
  const deleteData = async (id: string) => {
    await axios
      .delete("http://103.179.188.25:8000/students/" + id)
      .then((response) => {
        console.log(response.data);
        // setData(response.data);
      })
