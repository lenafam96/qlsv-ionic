...
const Home: React.FC = () => {
  ...
  const getData = async (sort: string = "", search: string = "") => {
    if (search === "") {
      if (sort === "asc") {
        await axios
          .get(`http://172.31.109.58:8000/students/score_asc`)
          .then((response) => {
            setData(response.data);
          });
      } else if (sort === "desc") {
        await axios
          .get(`http://172.31.109.58:8000/students/score_desc`)
          .then((response) => {
            setData(response.data);
          });
      } else {
        await axios
          .get(`http://172.31.109.58:8000/students/`)
          .then((response) => {
            setData(response.data);
          });
      }
      ...
      await axios
        .get(
          `http://172.31.109.58:8000/students/search?search=${search}&sort=${sort}`
        )
        .then((response) => {
          setData(response.data);
        });
